...
import type { LocalChapter, LocalManga } from '@/types'

export default function LocalReader() {
  const { chapterId } = useParams<{ chapterId: string }>()
  const [chapter, setChapter] = useState<LocalChapter | null>(null)
  ...
  useEffect(() => {
    ...
    axios.get(`/api/local-manga/chapter/${chapterId}`)
      .then(res => {
        const ch = res.data
        setChapter(ch)
        const mangaData = ch.mangaId as unknown as LocalManga
        setManga(mangaData)
        // Load all chapters for nav
        return axios.get(`/api/local-manga/${mangaData._id}/chapters`)
      })
      ...
      <div className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${showControls ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
        <div className="bg-gradient-to-b from-black/90 to-transparent p-4">
          <div className="max-w-3xl mx-auto flex items-center justify-between">
            <Link to={manga ? `/local/${manga._id}` : '/catalog'}
